...
#models = [DLT(),ExponentialSmoothing(), NaiveSeasonal(), NaiveDrift(), NaiveMean(), AutoARIMA(), Theta(), FFT(), Prophet(),Croston(),LightGBMModel(lags=1),RandomForest(lags= 1,random_state=2309),TBATS(use_box_cox=False),BATS(use_box_cox=False),RegressionModel(lags=1),RegressionModel(lags=10),XGBModel(lags=1)]#, StandardRegressionModel(),,RegressionEnsembleModel(forecasting_models=[ExponentialSmoothing(), NaiveSeasonal(),AutoARIMA(),TBATS(use_)],regression_train_n_points=24)]
#models_names = ['Orbit','ExponentialSmoothing', 'NaiveSeasonal','NaiveDrift','NaiveMean','AutoARIMA','Theta', 'FFT','Prophet','Croston','LightGBMModel','RandomForest','TBATS','BATS','RegressionModelL1','RegressionModelL10','XGBoost']#, 'StandardRegression',,'RegressionEnsembleModel']

models = [DLT(), NaiveSeasonal(), NaiveDrift(), NaiveMean(), Prophet(),BATS(use_box_cox=False),RegressionModel(lags=1),XGBModel(lags=1)]#, StandardRegressionModel(),,RegressionEnsembleModel(forecasting_models=[ExponentialSmoothing(), NaiveSeasonal(),AutoARIMA(),TBATS(use_)],regression_train_n_points=24)]
models_names = ['Orbit', 'NaiveSeasonal','NaiveDrift','NaiveMean','Prophet','BATS','RegressionModelL1','XGBoost']#, 'StandardRegression',,'RegressionEnsembleModel']
