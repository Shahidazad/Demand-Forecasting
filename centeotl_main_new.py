...
from centeotl_scripts import load_data, utils, pre_process , write_to_database as w2d #, get_ts_features , fit_models,
from centeotl_scripts.fit_models import  Darts, Orbit
...
#%% Setting up logging ###
logging.config.fileConfig(fname='params/log.conf', disable_existing_loggers=True)
# Get the logger specified in the file
logger = logging.getLogger(__name__)
logger.info('This is an info message')
#jhds_logger = logging.getLogger('huitzilo_main')
jhds_logger = logging.getLogger('jhdsLogger')
### End setup logging
...
def main():
    ...
    data = pd.read_parquet('data/centeotl_orders_data.parquet')
    ...
    if not test_run:
        ...
        data.to_parquet('data/centeotl_orders_data.parquet')
    ...
    #idx = pd.date_range(order_min_date,order_max_date,freq='MS',name='Order_Create_Date')
    #prep_inter_prod = pre_process.fix_dates(prep_init_prod,idx,keys)

    #%% Dataframe for modeling
    #gk = group_key.copy()
    #gk.extend(['counts','run_id','group_key'])
    #prep_comp_prod = pd.merge(prep_inter_prod,prep_init_prod['group_key'].\
    #    drop_duplicates(), on=['group_key'], how='inner')
    #prep_comp_prod = prep_inter_prod.copy()
    ...
    if len(prep_incomplete_prod)>0:
        ...
        inc_fcst_idx = pd.date_range(run_mofcst,order_max_date + relativedelta(months=12),freq='MS',name='cyear_month')
        ...
        if len(inc_keys)>0 :
            ...
            incompletes_fcst['lag'] = incompletes_fcst.groupby('group_key')['cyear_month'].cumcount()
        else:
        ...
        prep_incomplete_prod6_forecast_all.rename(columns={'level_1':'Order_Create_Date'},inplace=True)
