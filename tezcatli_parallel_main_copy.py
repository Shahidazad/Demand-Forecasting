...
# from preprocessing_data import keys,prep_comp_prod,prep_incomplete_prod
...
def main():
    ...
    # data['forecast_group']=data['group_key'].apply(lambda x:x.split('_')[1])
    # data['Region Name']=data['group_key'].apply(lambda x:x.split('_')[0])
    # data=data['Region Name'].isin(['Texas and South Plains','Southeast','Mid South','Carolinas','Southwest','Pacific Northwest','Mountain','Midwest East','Midwest West','Midwest Central','Mid Atlantic','North Atlantic'])
    # data=data.loc[data['forecast_group']=="Pr Soff 8'"] # IMP
    ...
    model_accs_dfs = []
    print('------1.2.5')
    with parallel_backend('threading',n_jobs=2): #n_jobs=4
        print('------1.2.5.1')
