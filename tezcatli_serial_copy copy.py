...
# prep_comp_prod.head()
prep_comp_prod=pd.read_parquet('data/tezcatli_orders_data.parquet')
prep_comp_prod=prep_comp_prod.loc[(prep_comp_prod['Region Name']=='North Atlantic') & (prep_comp_prod['forecast_group']=='C+ St Plk')]
prep_comp_prod['group_key']='North Atlantic_C+ St Plk'
from datetime import datetime
prep_comp_prod['run_id']= getrandbits(32)
prep_comp_prod['Order_Volume_(STD)']=prep_comp_prod['Order Volume (STD)']
prep_comp_prod['Order_Create_Date']=prep_comp_prod['Order Create Date']
prep_comp_prod=prep_comp_prod[['Order_Create_Date','Order_Volume_(STD)','group_key','run_id']]
input_dates=list(prep_comp_prod['Order_Create_Date'])
output_dates = []
for input_date_str in input_dates:
    ...
    output_dates.append(output_date_str)
prep_comp_prod['Order_Create_Date']=output_dates
...
#keys[key].item()
key = 'North Atlantic_C+ St Plk' 


# In[8]:


prod_df = prep_comp_prod.drop_duplicates()
# prod_df=prod_df.sort_values(by='Order_Create_Date')
# prod_df=prod_df.drop_duplicates()
# prod_df.reset_index(inplace=True,drop=True)
# Extract year and month columns
prod_df['Order_Create_Date']=pd.to_datetime(prod_df['Order_Create_Date'])
prod_df['year'] = prod_df['Order_Create_Date'].dt.year
prod_df['month'] = prod_df['Order_Create_Date'].dt.month


# Group by year and month, then calculate the sum of 'quantity' column
prod_df= prod_df.groupby(['year', 'month'])['Order_Volume_(STD)'].sum().reset_index()
prod_df['Order_Create_Date']=prod_df['year'].astype('string')+'-'+prod_df['month'].astype('string')+'-01'
prod_df['run_id']= getrandbits(32)
prod_df['group_key']='North Atlantic_C+ St Plk'
prod_df.drop(['year','month'],axis=1,inplace=True)
prod_df=prod_df[['Order_Create_Date','Order_Volume_(STD)','group_key','run_id']]
prod_df['Order_Create_Date']=pd.to_datetime(prod_df['Order_Create_Date'])
print('prod_df_final',prod_df.dtypes)
# In[9]:
...
print('model_dfs',model_dfs)
print('model_accs',model_accs)
